    ...
    def get_stats(self, operation: str) -> Optional[Dict[str, Any]]:
        ...
        if operation in self.memory_usage and self.memory_usage[operation]:
            ...
            if 'cpu_percent' in mem_records[-1]:
                cpu_values = [r['cpu_percent'] for r in mem_records if 'cpu_percent' in r]
                if cpu_values:
                    stats['avg_cpu_percent'] = sum(cpu_values) / len(cpu_values)
            
            if 'memory_mb' in mem_records[-1]:
                mem_values = [r['memory_mb'] for r in mem_records if 'memory_mb' in r]
                if mem_values:
                    stats['avg_memory_mb'] = sum(mem_values) / len(mem_values)
            
            if 'gpu_memory_mb' in mem_records[-1]:
                gpu_values = [r['gpu_memory_mb'] for r in mem_records if 'gpu_memory_mb' in r]
                if gpu_values:
                    stats['avg_gpu_memory_mb'] = sum(gpu_values) / len(gpu_values)
    ...
    def save_to_file(self, filename: str = "timing_report.txt"):
        ...
        with open(filename, 'w') as f:
            ...
            for operation in sorted_ops:
                ...
                f.write(f"  Min/Max:    {stats['min']:.3f}s / {stats['max']:.3f}s\n")
                ...
                if 'avg_gpu_memory_mb' in stats:
                    f.write(f"  Avg GPU Mem: {stats['avg_gpu_memory_mb']:.1f} MB\n")
                f.write("\n")
...
.        print(f"\n⏱️  Starting: {operation_name}...")
    ...
    try:
        ...
        if verbose:
            ...
            msg_parts = [f"⏱️  [{operation_name}] Completed in {duration:.3f}s"]
